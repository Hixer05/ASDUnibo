...
public class DHeap<K extends Comparable<K>,D> implements PriorityQueue<K,D>{
	...
	*/
	public PriorityQueueNode<K,D> insert(K key, D data) {
        DHeapNode<K,D> node = new DHeapNode<K,D>(key, data, n);
        nodes.add(node);
        n++;
        moveUp(node);
        return node;
	...
	*/
	public void delete(PriorityQueueNode<K,D> node) {
        DHeapNode<K,D> u = nodes.remove(n-1);
        n--;
        if(u!=node){
            int i = ((DHeapNode<K,D>) node).index;
            nodes.set(i,u);
            u.index = i;
            moveUp(u);
            moveDown(u);
	...
	*/	
	public void increaseKey(K newKey, PriorityQueueNode<K,D> node) {
        if(newKey.compareTo(node.getKey()) >0){
            DHeapNode<K,D> dnode = (DHeapNode<K,D>) node;
            dnode.key = newKey;
            moveDown(dnode);
        }
	}
